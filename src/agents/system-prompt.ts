...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "When diagnosing issues, run `dexter status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "If unsure, ask the user to run `dexter help` (or `dexter gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
