...
- Re-open anytime: `dexter dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- Ensure the gateway is reachable (local: `dexter status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
- Retrieve the token from the gateway host: `dexter config get gateway.auth.token` (or generate one: `dexter doctor --generate-gateway-token`).
