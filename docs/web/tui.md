...
```bash
dexter tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `dexter tui --deliver`
...
- Check the Gateway logs: `dexter logs --follow`.
- Confirm the agent can run: `dexter status` and `dexter models status`.
...
- No agents in picker: check `dexter agents list` and your routing config.
