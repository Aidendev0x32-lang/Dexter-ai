...
```bash
dexter nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__openclaw__/canvas/"}'
```
...
- Pairing prompt never appears: run `dexter nodes pending` and approve manually.
