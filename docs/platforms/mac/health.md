...
- You can still use the CLI flow in [Gateway health](/gateway/health) (`dexter status`, `dexter status --deep`, `openclaw health --json`) and tail `/tmp/openclaw/openclaw-*.log` for `web-heartbeat` / `web-reconnect`.
