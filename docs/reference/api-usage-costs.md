...
- `dexter status --usage` and `dexter channels list` show provider **usage windows**
...
`dexter models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
