...
`dexter onboard`、`dexter configure` 或 `dexter setup` 将创建工作区并在缺失时填充引导文件。
...
`dexter doctor` 在检测到额外工作区目录时会发出警告。
...
`dexter setup` 可以重新创建缺失的默认值而不覆盖现有文件。
...
3. 运行 `dexter setup --workspace <path>` 来填充任何缺失的文件。
