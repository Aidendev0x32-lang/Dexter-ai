...
```bash
dexter plugins enable minimax-portal-auth  # 如果已加载则跳过
dexter gateway restart  # 如果 Gateway 网关已在运行则重启
...
- 使用 `dexter models list` 和 `dexter models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `dexter configure` 并选择 **MiniMax M2.1**，或
