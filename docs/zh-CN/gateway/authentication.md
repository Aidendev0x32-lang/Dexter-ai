...
2. 将其放在 **Gateway 网关主机**（运行 `dexter gateway` 的机器）上。
...
如果你不想自己管理环境变量，新手引导向导可以为守护进程使用存储 API 密钥：`dexter onboard`。
...
运行 `dexter models status` 确认哪个配置文件即将过期。如果配置文件缺失，重新运行 `claude setup-token` 并再次粘贴令牌。
