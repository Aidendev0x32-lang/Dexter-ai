...
- 服务状态：`dexter gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均支持）：

- 向导（推荐）：`dexter onboard --install-daemon`
- 直接安装：`dexter gateway install`
- 配置流程：`dexter configure` → 选择 **Gateway service**
- 修复/迁移：`dexter doctor`（提供安装或修复服务）
