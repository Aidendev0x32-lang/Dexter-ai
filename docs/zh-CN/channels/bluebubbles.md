...
3. 运行 `dexter onboard` 并选择 BlueBubbles，或手动配置：
   ```json5
   {
     channels: {
       bluebubbles: {
...
- 配对码在一小时后过期；使用 `dexter pairing list bluebubbles` 和 `dexter pairing approve bluebubbles <code>`。
...
- 查看状态/健康信息：`dexter status --all` 或 `dexter status --deep`。
