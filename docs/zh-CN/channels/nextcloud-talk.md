...
```bash
dexter plugins install @openclaw/nextcloud-talk
```

本地检出安装（从 git 仓库运行时）：

```bash
dexter plugins install ./extensions/nextcloud-talk
```
...
- 批准方式：
  - `dexter pairing list nextcloud-talk`
  - `dexter pairing approve nextcloud-talk <CODE>`
