...
```bash
dexter plugins enable minimax-portal-auth  # skip if already loaded.
dexter gateway restart  # restart if gateway is already running
dexter onboard --auth-choice minimax-portal
```
...
## Configure via `dexter configure`

Use the interactive config wizard to set MiniMax without editing JSON:

1. Run `dexter configure`.
2. Select **Model/auth**.
...
- Use `dexter models list` and `dexter models set minimax/MiniMax-M2.1` to switch.
...
- Running `dexter configure` and selecting **MiniMax M2.1**, or
