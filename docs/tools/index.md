...
- `host=node` can target a macOS companion app or a headless node host (`dexter node run`).
...
- Requires a Brave API key (recommended: `dexter configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `dexter nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `dexter gateway` restart in-place)
