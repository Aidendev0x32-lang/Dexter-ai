...
- `openclaw health` and `dexter status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `dexter gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.
